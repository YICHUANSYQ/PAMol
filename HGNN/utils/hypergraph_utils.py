...
def _generate_G_from_H(H, variable_weight=False):
    ...
    maskDE = (DE != 0)
    reciprocal_DE = np.zeros_like(DE)
    reciprocal_DE[maskDE] = 1 / DE[maskDE]
    invDE = np.mat(np.diag(reciprocal_DE))
    maskDV = (DV != 0)
    reciprocal_DV = np.zeros_like(DV)
    reciprocal_DV[maskDV] = 1 / DV[maskDV]
    DV2 = np.mat(np.diag(np.power(reciprocal_DV, 0.5)))
    W = np.mat(np.diag(W))
    H = np.mat(H)
    HT = H.T
